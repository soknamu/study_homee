...
from tensorflow.keras.layers import Dense, Embedding, Dropout, Flatten,Conv1D
model = Sequential()
model.add(Embedding(30, 32,input_length=6)) #얘는 Flatten 됨. 모양이()
#model.add(Embedding(30, 32)) #Flatten에 바로 붙이지 못한다.
model.add(Conv1D(128,2)) #커널사이즈(2*2)로 짜르겠다.
model.add(Flatten())
